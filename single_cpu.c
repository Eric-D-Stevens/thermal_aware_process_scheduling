...
int no_function(char *stress_call){
    //sleep(seconds);    
    printf("\n\n Start stress: ");
    char command_string[50] = "taskset -c 0 ";
    strcat(command_string, stress_call);
...
int main(int argc, char *argv[]){
    ...
    printf("\n\n\nCALL: %s", stress_call);
    printf("\nFNAME: %s\n\n", out_name);
    sprintf(out_filename, "data0/%s", out_name);
    printf("\n\nOFN: %s\n\n",out_filename);
    //exit(0);
	...
	for(int i=0; i<num_hw_cache_events; i++)
	{
		...
		if(i+num_hw_events==0){fd_arr[i+num_hw_events] = perf_event_open(&pat_arr[i+num_hw_events],0,0,-1,0);}
		else{fd_arr[i+num_hw_events] = perf_event_open(&pat_arr[i+num_hw_events],0,0,fd_arr[0],0);}
